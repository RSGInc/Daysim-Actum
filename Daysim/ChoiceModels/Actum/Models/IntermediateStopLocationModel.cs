...
namespace Daysim.ChoiceModels.Actum.Models {
	public class IntermediateStopLocationModel : ChoiceModel {
		...
		private sealed class IntermediateStopLocationUtilities : ISamplingUtilities {
			...
			private float WTHRESH = 0.25F * (float) Global.Settings.DistanceUnitsPerMile; //if trip is less than this distance, then walk LOS is assumed  (distance units)
			private double DLIMPARM = 30.0 * Global.Settings.DistanceUnitsPerMile; // soto distance limit (distance units) at which gtim sensivity stops changing
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				var household = (HouseholdWrapper) _trip.Household;
				var person = (PersonWrapper) _trip.Person;
				var tour = (TourWrapper) _trip.Tour;
				...
				// stop purpose
				var businessDestinationPurposeFlag = _trip.IsBusinessDestinationPurpose().ToFlag();
				var schoolStopForChildUnderAge16 = (_trip.IsSchoolDestinationPurpose() && (person.IsChildAge5Through15 || person.IsChildUnder5)).ToFlag();
				var schoolStopForDrivingAgeStudent = (_trip.IsSchoolDestinationPurpose() && person.IsDrivingAgeStudent).ToFlag();
				var schoolStopForAdult = (_trip.IsSchoolDestinationPurpose() && (!person.IsChildAge5Through15 && !person.IsChildUnder5) && !person.IsDrivingAgeStudent).ToFlag();
				//var workOrSchoolDestinationPurposeFlag = _trip.IsWorkOrSchoolDestinationPurpose.ToFlag();
				var escortStop_HouseholdHasChildren = (_trip.IsEscortDestinationPurpose() && (household.HouseholdTotals.DrivingAgeStudents + household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 > 0)).ToFlag();
				var escortStop_HouseholdHasNoChildren = (_trip.IsEscortDestinationPurpose() && (household.HouseholdTotals.DrivingAgeStudents + household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 == 0)).ToFlag();
				//#if TRACE
				var personalBusinessStopPurposeFlag = _trip.IsPersonalBusinessDestinationPurpose().ToFlag();
				//#endif
				var shoppingStopPurposeFlag = _trip.IsShoppingDestinationPurpose().ToFlag();
				var mealStopPurposeFlag = _trip.IsMealDestinationPurpose().ToFlag();
				var socialStopPurposeFlag = _trip.IsSocialDestinationPurpose().ToFlag();
				var recreationStopPurposeFlag = _trip.IsRecreationDestinationPurpose().ToFlag();
				//var srec = (socialDestinationPurposeFlag == 1 || recreationDestinationPurposeFlag == 1).ToFlag();
				var medicalStopPurposeFlag = _trip.IsMedicalDestinationPurpose().ToFlag();
				var personalBusiness_Meal_Shop_SocialStopPurposeFlag = _trip.IsPersonalReasonsDestinationPurpose().ToFlag();
				var personalBusinessOrMedicalStopPurposeFlag = _trip.IsPersonalBusinessOrMedicalDestinationPurpose().ToFlag();

				// tour and trip characteristics
				var escortStopOnSchoolTour = (_trip.IsEscortDestinationPurpose() && tour.IsSchoolPurpose()).ToFlag();
				var shoppingStopOnShoppingTour = (_trip.IsShoppingDestinationPurpose() && tour.IsShoppingPurpose()).ToFlag(); // shoppingDestinationPurposeFlag trip on shoppingDestinationPurposeFlag tour
				var stopIsBeforeMandatoryTourDestination = (_trip.IsHalfTourFromOrigin && (tour.IsWorkPurpose() || tour.IsSchoolPurpose())).ToFlag();
				var parkingTime = _averageStopDuration[_trip.DestinationPurpose] / 6000D; // parking time (hrs)
				var notFirstStopOnHalfTourFlag = (_trip.Sequence != 1).ToFlag();

				var employmentGovernment_Office_Education = destinationParcel.EmploymentGovernment + destinationParcel.EmploymentOffice + destinationParcel.EmploymentEducation;
				var employmentIndustrial_Ag_Construction = destinationParcel.EmploymentIndustrial + destinationParcel.EmploymentAgricultureConstruction;
				var studentsK12 = destinationParcel.StudentsK8 + destinationParcel.StudentsHighSchool;

				var logOfOnePlusEmploymentServiceBuffer1 = Math.Log(destinationParcel.EmploymentServiceBuffer1 + 1);
				var logOfOnePlusEmploymentMedicalBuffer1 = Math.Log(destinationParcel.EmploymentMedicalBuffer1 + 1);
				var logOfOnePlusEmploymentRetailBuffer1 = Math.Log(destinationParcel.EmploymentRetailBuffer1 + 1);
				var logOfOnePlusEmploymentFoodBuffer1 = Math.Log(destinationParcel.EmploymentFoodBuffer1 + 1);
				var logOfOnePlusEmploymentGovernment_Office_EducationBuffer1 = Math.Log(destinationParcel.EmploymentGovernmentBuffer1 + destinationParcel.EmploymentOfficeBuffer1 + destinationParcel.EmploymentEducationBuffer1 + 1);
				var logOfOnePlusEmploymentIndustrial_Ag_ConstructionBuffer1 = Math.Log(destinationParcel.EmploymentIndustrialBuffer1 + destinationParcel.EmploymentAgricultureConstructionBuffer1 + 1);
				var logOfOnePlusEmploymentTotalBuffer1 = Math.Log(destinationParcel.EmploymentTotalBuffer1 + 1);
				var logOfOnePlusHouseholdsBuffer1 = Math.Log(destinationParcel.HouseholdsBuffer1 + 1);
				var logOfOnePlusStudentsK12Buffer1 = Math.Log(destinationParcel.StudentsK8Buffer1 + destinationParcel.StudentsHighSchoolBuffer1 + 1);
				var logOfOnePlusStudentsUniversityBuffer1 = Math.Log(destinationParcel.StudentsUniversityBuffer1 + 1);
				var logOfOnePlusParkingOffStreetPaidHourlySpacesBuffer1 = Math.Log(destinationParcel.ParkingOffStreetPaidHourlySpacesBuffer1 + 1);
				var openSpaceType2IsPresentInBuffer1 = (destinationParcel.OpenSpaceType2Buffer1 > 0).ToFlag();
				var numberOfNetworkNodesInBuffer1 = destinationParcel.NodesSingleLinkBuffer1 + destinationParcel.NodesThreeLinksBuffer1 + destinationParcel.NodesFourLinksBuffer1;

				var stopOnJointTour = (tour.JointTourSequence > 0).ToFlag();
				var stopOnFullJointHalfTour = ((_trip.Direction == Global.Settings.TourDirections.OriginToDestination && tour.FullHalfTour1Sequence > 0)
				...
				var costCoef = tour.CostCoefficient;
				var timeCoef = tour.TimeCoefficient;
				var transitDiscountFraction = tour.Person.GetTransitFareDiscountFraction();
				var minute = DayPeriod.BigDayPeriods[DayPeriod.MIDDAY].Start;
				var purpose = tour.DestinationPurpose;
				...
				var dist = Math.Max(adis1 + adis2 - adis0, Math.Min(adis1 * 0.1, adis2 * 0.1)); // incremental distance (with imposed lower limit), in units of miles
				var gtime_DistanceSensitiveDetourGeneralizedTimeHundredsOfMinutes = (gtim1 + gtim2) / 100.0; // approximate incremental gtime, rescale to units of (100 minutes)  //rescale to units of (100 minutes
				...
				var gtimeSquared = gtime_DistanceSensitiveDetourGeneralizedTimeHundredsOfMinutes * gtime_DistanceSensitiveDetourGeneralizedTimeHundredsOfMinutes;
				var gtimeCubed = gtimeSquared * gtime_DistanceSensitiveDetourGeneralizedTimeHundredsOfMinutes;
				var ttim = Math.Min((ttim1 + ttim2) / 100.0, 3.6); // rescale to units of (100 minutes)
				var logOfOneMinusRatioOfTravelTimeToAvailableTimeWindow = ttim / (Math.Max(1.0, _availableWindow) / 100D); // unit-free ratio of ttim and window, both measured in 100 minute units
				...
				//var detourDistanceCubedInHundredsOfMilesCubed = Math.Min(0.125, diss * dist);
 				var maxDist = .5 * Global.Settings.DistanceUnitsPerMile;  // in hundreds of distance units
				var diss = Math.Min(maxDist * maxDist, dist * dist);
				var detourDistanceCubedInHundredsOfDistanceUnitsCubed = Math.Min(maxDist * maxDist * maxDist, diss * dist);
				...
				var walkAndTransitAreInaccessibleForOneLegOfDetour = 0;
				var walkAndTransitAreInaccessibleForBothLegsOfDetour = 0;
				var wta = 0;
				...
				var adultFemaleWithChildren = (person.Gender == Global.Settings.PersonGenders.Female && person.PersonType < Global.Settings.PersonTypes.DrivingAgeStudent && (household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 + household.HouseholdTotals.DrivingAgeStudents > 0)).ToFlag();
				//#if TRACE
			...
			private static void GetGenTime(IRandomUtility randomUtility, int mode, int purpose, double costCoef, double timeCoef, double transitDiscountFraction, int transitPassOwnership, int leg, IParcelWrapper tripOrigin, IParcelWrapper tripDestination, IParcelWrapper tourOrigin, out double dis, out double tim, out double gtim, int minute = -1, double parkHours = 0.0) {
				...
				if (path.Available) {
					...
					if (parkHours > 0) {
						var pcost = tripDestination.ParkingCostBuffer1(parkHours);
